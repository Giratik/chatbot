"""
rag_app.py — Page Streamlit RAG intégrée
Adaptée depuis app.py fourni par l'utilisateur.
"""
...
def render_sidebar(chroma_client, ollama_client) -> dict:
    ...
    with st.sidebar:
        ...
        use_hyde = st.checkbox("HyDE (réponse hypothétique)", value=True)
        use_expansion = st.checkbox("Query expansion (synonymes)", value=True)
        alpha = st.slider("Vectoriel ← → BM25", 0.0, 1.0, 0.5, 0.05)

        st.markdown("---")
        st.markdown("**🎯 Reranking**")
        use_reranker = st.checkbox("Reranker (bge-reranker-v2-gemma)", value=True)
